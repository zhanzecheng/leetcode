...
class Solution(object):
    def minMutation(self, beginWord, endWord, wordList):
        """
        :type beginWord: str
        :type endWord: str
        ...
        while queue.qsize() > 0:
            tmp_count, tmp_cur = queue.get()
            if tmp_cur == endWord:
                return tmp_count + 1

            for i in range(length_word):
                for c in 'abcdefghijklmnopqrstuvwxyz':
                    next_word = tmp_cur[:i] + c + tmp_cur[i + 1:]
                    if next_word in wordList:
                        wordList.remove(next_word)
                        queue.put([tmp_count + 1, next_word])
        return 0
...
if __name__ == '__main__':
    ...
    be = "sand"
    en = "acne"
    data = ["slit", "bunk", "wars", "ping", "viva", "wynn", "wows", "irks", "gang", "pool", "mock", "fort", "heel", "send",
    ...
    print(solution.ladderLength(be, en, data))